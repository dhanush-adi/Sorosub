...
export default function MyWallet() {
  ...
  // Fetch live XLM balance from Horizon API
  useEffect(() => {
    const fetchLiveBalance = async () => {
      ...
      try {
        ...
        if (xlmBalance) {
          setLiveBalance(parseFloat(xlmBalance.balance))
        } else {
          setLiveBalance(0)
        }

        console.log('[MyWallet] Live XLM balance:', xlmBalance?.balance)
      } catch (error) {
        console.error('[MyWallet] Error fetching balance:', error)
        setLiveBalance(null)
      } finally {
  ...
  // Dynamic assets based on real Horizon balance
  const assets: Asset[] = liveBalance !== null ? [
    {
      id: '1',
      name: 'Stellar Lumens',
      symbol: 'XLM',
      balance: liveBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 7 }),
      value: `$${(liveBalance * 0.12).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, // ~$0.12 per XLM
      change: '+0.0%',
      trend: 'up',
    },
  ] : []
  ...
  // Use live balance from Horizon API, fallback to hook balance
  const displayBalance = liveBalance !== null ? liveBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : balance.toFixed(2)
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-baseline gap-3 flex-wrap">
                  <span className="text-4xl md:text-5xl font-bold">
                    {showBalance ? `${displayBalance} XLM` : '••••••'}
                  </span>
                  <button
